...
namespace Inworld.Entities.LLM
{
	...
	public class CompleteChatRequest
	{
		...
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
		public ToolChoice tool_choice; // Controls which (if any) function is called by the model.
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
		...
		public ResponseFormat response_format = ResponseFormat.RESPONSE_FORMAT_JSON; // Format that the model must output. Used to enable JSON mode.

		public CompleteChatRequest()
		{
			
		}
		public CompleteChatRequest(Serving serving, List<Message> messages, TextGenerationConfig config = null)
		{
			serving_id = serving;
